...
function get_wip( &$r_wip_data) {
	...
	(select to_char(ll.crt_dts, 'yyyy-mm-dd hh24:mi:ss') from lot_log ll where ll.lot_id = lbs.lot_id_str and ll.verb = 'PDCR') crt_dts
	...
	where lbs.lot_st_dvs not in ('PD','CL') and lbs.chp_qty!='0' and lbs.lot_id not in (select lot_id from lot_inf_tbl  where ctg_cd='CT00S0000277')
		...
		$w_tmp_data_row .= date('Y-m-d H:i:s',strtotime($w_row['CRT_DTS']));
