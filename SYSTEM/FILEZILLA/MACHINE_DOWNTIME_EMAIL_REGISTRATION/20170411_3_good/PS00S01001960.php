...
function check_input($w_mode)
{
	...
	switch ($w_mode) {
		...
		case 2:
			...
			echo $blank_eqp_email = $gw_scr['s_inp_row'] - ($blank_email_eqp-1);
			echo $blank_tnm_email = $gw_scr['s_inp_row'] - ($blank_email_tnm-1);
			echo $blank_prn_email = $gw_scr['s_inp_row'] - ($blank_email_prn-1);

			if($blank_eqp_email == 0 && $blank_tnm_email == 0 && $blank_prn_email == 0){
				$g_msg = xpt_err_msg($g_msg, itm("Email"), __LINE__);
				return 4000;
			}
		break;
	}
