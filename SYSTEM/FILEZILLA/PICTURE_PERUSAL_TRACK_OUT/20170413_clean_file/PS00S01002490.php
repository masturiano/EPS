...
# [PRC_ID]:                     [PROCESS] :
# [PGM_ID]: PS00S01000400.php   [PROGRAM] : 次工程渡し受入画面(組立) 
# [MDL_ID]:                     [MODULE]  :
# -------------------------------------------------------------------------------
# [COMMENT] 変数定義
...
require_once ($g_lang_dir . "/PS00S01000400M.php");			# メッセージ
#require_once ($g_scr_dir  . "/PS00S01000400S.php");			# スクリーン定義
...
#	global $g_stp_no;			# ステップ番号
	...
	if($r_lot_bas[LOT_ST_DVS] == "OW") {
		...
		if($rtn == -1){
			list($g_msg, $g_err_lv) = PS00S01000400_msg("err_Nxt_Prc");
			$g_msg = xpt_err_msg($g_msg, "", __LINE__);
			return 4000;
		}
	} else {
        list($g_msg, $g_err_lv) = PS00S01000400_msg("err_LotState");
		$g_msg = xpt_err_msg($g_msg, "", __LINE__);
		return 4000;
	}
	if ($rtn) {
...
#	global $g_lot_bas;				# ロット基本情報データ(連想配列)
#	global $g_nm;
#	global $g_bd_dvs;				# 保税区分名
#	global $g_bnd_dvs;				# 保税区分
	...
	$w_rtn = xgt_stp_cls($w_next_dat['STP_CD'], $w_stpcls2, $dmy);
        ...
        {
		list ($g_msg, $g_err_lv) = PS00S01000400_msg("err_LotStepPlating");
	        $g_msg= xpt_err_msg($g_msg,"", __LINE__);
        	return 4000;
	...
	if(!check_eisu($gw_scr['s_usr_id'])) {
        list($g_msg, $g_err_lv) = PS00S01000400_msg("err_Alphabet");
		$g_msg = xpt_err_msg($g_msg, PS00S01000400_item("UsrID"), __LINE__);
		return 4000;
	}
	# チェック：禁則文字
	if(!check_err_lot($gw_scr['s_lot_id'])) {
        list($g_msg, $g_err_lv) = PS00S01000400_msg("err_Irregular");
		$g_msg = xpt_err_msg($g_msg, PS00S01000400_item("LotID"), __LINE__);
		return 4000;
	}
...
#	$gw_scr['s_usr_nm'] = $g_usr_nm;	//戻り値
	//$gw_scr['g_usr_id'] = $gw_scr['g_usr_id'];	//グローバルID更新
	...
	*/
	...
	if(in_array($w_stpcls2,unserialize(constant("E9_NG_STEPS")))){
		list($g_msg, $g_err_lv) = PS00S01000400_msg("err_LotStepPacking");
		$g_msg = xpt_err_msg($g_msg, "", __LINE__);
		return 4000;
	}
...
#	$gw_scr["s_bd_dvs"] = $g_bd_dvs;	//戻り値
	...
	$w_rtn = get_stp_val($w_lot_bas['RT_CD'],$w_lot_bas['STP_CD'],$w_st_stp);
        if($w_rtn != 0){
                list($g_msg, $g_err_lv) = PS00S01000400_msg("err_sel_dvs");
                $g_msg = xpt_err_msg($g_msg, "", __LINE__);
                return 4000;
        }
        ...
        if($w_st_stp['val'] == 'S'){
                ...
                if($w_rtn != 0){
                        list($g_msg, $g_err_lv) = PS00S01000400_msg("err_no_end_step");
                        $g_msg = xpt_err_msg($g_msg, "", __LINE__);
                        return 4000;
                }
                ...
                if($w_rtn != 0){
                        list($g_msg, $g_err_lv) = PS00S01000400_msg("");
                        $g_msg = xpt_err_msg($g_msg, "", __LINE__);
                        return 4000;
                }
...
#		$g_upd_lev = $g_lot_bas[UPD_LEV];
	...
	### ホールド時のメッセージ
	if($w_hold_exc_flg == 1){
		list($w_hdmsg, $w_hdlv) = PS00S01000400_msg("End_Rsv_Hold");
		$w_hdmsg .= "<br>";
		$w_hdmsg .= PS00S01000400_item("RsvHoldInfo");
...
function extra_iomv($w_rt_cd, $w_curr_stp, &$w_lot_bas){	
        ...
        if($w_st_stp['val'] == 'S'){
                ...
                if($w_rtn != 0){
                        list($g_msg, $g_err_lv) = PS00S01000400_msg("err_sel_stps");
                        $g_msg = xpt_err_msg($g_msg, "", __LINE__);
                        return 4000;
                }
...
SQL;
        ...
        $w_rtn = db_do($w_stmt);
        if($w_rtn != 0){
                list($g_msg, $g_err_lv) = PS00S01000400_msg("err_Sel");
                $g_msg = xpt_err_msg($g_msg, "", __LINE__);
                return 4000;
        }
...
function main_read_fuc()
{
	...
	if (check_input()) {
		...
		# MODE2画面表示
		if(!$g_msg){
			list($g_msg, $g_err_lv) = PS00S01000400_msg("comp_CHECK");
		}
		scr_mode_chg(1);
	}
...
function main_update_fuc()
{
	...
	if (check_update()) {
		...
		# MODE3画面表示
		if(!$g_msg){
			list($g_msg, $g_err_lv) = PS00S01000400_msg("comp_UPDATE");
		}
		scr_mode_chg(2);
	}
