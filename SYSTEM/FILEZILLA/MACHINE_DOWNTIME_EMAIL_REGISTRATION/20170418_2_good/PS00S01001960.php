	...
	switch($gw_scr['s_act']){
		...
		case "SEARCH";
			...
			if($w_rtn != 0){
				...
				$count_par_mst = count_par_mst($par_id, &$r_result);
				if($r_result == 0){
					scr_mode_chg(2);
				}
				else{
...
_SQL;
					...
					while($w_row = db_fetch_row($w_stmt)){
						...
						$row_par_txt = trim($w_row['PAR_TXT']);
						# For department
						$d_sql = <<<_SQL
...
where um.del_flg='0' 
and  '610' || substr(trim(um.usr_id),7,5)= '{$row_par_txt}' 
and nm.nm_g_sht in ('EQP','TNM','PRN')
and rownum = 1
order by nm.nm_g_sht
_SQL;
						$d_stmt = db_res_set($d_sql);
						$d_rtn  = db_do($d_stmt);
						if ($d_rtn != 0){
							list($g_msg, $g_err_lv) = msg("err_Sel_DepMst");
							$g_msg = xpt_err_msg($g_msg, "", __LINE__);
							return $d_rtn;
						}
						if($d_row = db_fetch_row($d_stmt)){
							$department_code = trim($d_row['DEPARTMENT']);
							if($department_code == "EQP"){
								# Pass data to text row
								$pass_to_eqp_row[] = trim($w_row['PAR_TXT']);
							}
							if($department_code == "TNM"){
								# Pass data to text row
								$pass_to_tnm_row[] = trim($w_row['PAR_TXT']);
							}
							if($department_code == "PRN"){
								# Pass data to text row
								$pass_to_prn_row[] = trim($w_row['PAR_TXT']);
							}
						}
						# Pass data to text row
						//$gw_scr['s_lst_eqp_id_'][$row_increment] = trim($w_row['PAR_TXT']);
					}
					# Pass data to per department row
					$row_num_dept = 1;
					# EQP
					$count_pass_to_eqp_row = count($pass_to_eqp_row);
					if($count_pass_to_eqp_row > 0){
						foreach($pass_to_eqp_row as $value){
							$row_num_dept_eqp_increment = $row_num_dept++;
							$gw_scr['s_lst_eqp_id_'][$row_num_dept_eqp_increment] = trim($value);
						}
					}
					$row_num_dept = 1;
					# TNM
					$count_pass_to_tnm_row = count($pass_to_tnm_row);
					if($count_pass_to_tnm_row > 0){
						foreach($pass_to_tnm_row as $value){
							$row_num_dept_tnm_increment = $row_num_dept++;
							$gw_scr['s_lst_tnm_id_'][$row_num_dept_tnm_increment] = trim($value);
						}
					}
					$row_num_dept = 1;
					# PRN
					$count_pass_to_prn_row = count($pass_to_prn_row);
					if($count_pass_to_prn_row > 0){
						foreach($pass_to_prn_row as $value){
							$row_num_dept_prn_increment = $row_num_dept++;
							$gw_scr['s_lst_prn_id_'][$row_num_dept_prn_increment] = trim($value);
						}
					}
...
# Count par_mst first
function count_par_mst($par_id, &$r_result)
{
		global $g_msg;
		global $g_err_lv;
...
_SQL;
	...
	# Check for update the DEL_FLG
	$count_par_master_value = count($par_master_value);
	if($count_par_master_value > 0){
		foreach($par_master_value as $value){
			if (!in_array($value, (array)$integer_email_merge)){
				$subject_to_update[] = $value;
			}
		}
	}

	$trigger_for_insert = count($subject_to_insert);
	if($trigger_for_insert > 0){
		# Insert unique data
		...
		if($w_rtn != '4000') {
			list($g_msg, $g_err_lv) = msg("suc_Ins_User_id");
			$g_msg = xpt_err_msg($g_msg, "PAR_MST", "");
			db_commit();
			if($count_par_master_value > 0){
				main_md3_del_flag($subject_to_update);
			}
			scr_mode_chg(4);
		}
	}
	else{
		//list($g_msg, $g_err_lv) = msg("suc_Ins_User_id_Exist_All");
		list($g_msg, $g_err_lv) = msg("suc_Ins_User_id");
		$g_msg = xpt_err_msg($g_msg, "PAR_MST", "");
		if($count_par_master_value > 0){
			main_md3_del_flag($subject_to_update);
...
# ¥â¡¼¥É£±
#==================================================================
function main_md3_del_flag($subject_to_update){
	global $gw_scr;
	global $g_msg;
	global $g_err_lv;
	...
	$count_subject_to_update = count($subject_to_update);
	if($count_subject_to_update > 0){
		foreach($subject_to_update as $value){
			# Field to update
			$u_upd = array(
				"DEL_FLG" => "1"
			);
			...
			";
			# Execution
	        $u_rtn = db_update_one( "PAR_MST", $u_upd, $u_whr );
	        if ( $u_rtn != 0 ) {
	            list( $g_msg, $g_err_lv ) = msg( "err_Upd_ParMst" );
	            $g_msg = xpt_err_msg( $g_msg, "PAR_MST", __LINE__ );
	            return 4000;
	        }
	        db_commit();
...
function check_hrdata( $w_usr_ids, &$r_resp ){
	...
	$w_usr_ids = implode(',', (array)$w_usr_ids); // put array
...
function check_department( $w_usr_ids, &$r_resp ){

	foreach($w_usr_ids as $value){
	# For department
		$d_sql = <<<_SQL
select distinct(nm.nm_g_sht) as DEPARTMENT
from usr_mst um 
...
join nm_mst nm on nm.del_flg=0 and nm.cd=usr_grp_cd
where um.del_flg='0' 
and  '610' || substr(trim(um.usr_id),7,5)= '{$value}' 
and nm.nm_g_sht in ('EQP','TNM','PRN')
and rownum = 1
order by nm.nm_g_sht
_SQL;
		...
		$d_row = db_fetch_row($d_stmt);
		$d_row_count = count($d_row);
		if($d_row_count == 0){
			$no_department[] = $value;
		}
	}
	$r_resp = $no_department;
	...
	switch ($w_mode) {
		...
		case 2:
			...
			if($r_response['status'] == "invalid"){ // valid or invalid
				...
				$list_user_id = implode(',', (array)$invalid_ids); // put array
			...
			# Check department
			$temp_arr = $integer_email_merge;
			$w_rtn = check_department( $temp_arr, $r_response );
			if ( $w_rtn != 0 ) {
				list($g_msg, $g_err_lv) = msg("err_chk_dep_data");
				$g_msg = xpt_err_msg($g_msg, "", __LINE__);
				return 4000;
			}
			$no_department_count = count($r_response);
			$no_department = implode(',', (array)$r_response); // put array
			if($no_department_count > 0){
				list($g_msg, $g_err_lv) = msg("err_chk_Dept_NotFound");
				$g_msg = xpt_err_msg($g_msg, $no_department, __LINE__);
				return 4000;
			}
