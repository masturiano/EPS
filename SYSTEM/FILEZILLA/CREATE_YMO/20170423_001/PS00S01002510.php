...
# [SUB_ID]:						[SUBSYS]  : 
# [PRC_ID]:						[PROCESS] : 
# [PGM_ID]: PS00S01000650.php	[PROGRAM] : Create YMO Lot
# [MDL_ID]:						[MODULE]  : 
# --------------------------------------------------------------------------------------
# [COMMENT]
#
...
# ====================	==================	============================================
# DOS)Y.Nishigami       2014.04.24          UTAC対応 入力されたロットにCCSEM10とそれ以外が混在すればエラー
# --------------------------------------------------------------------------------------
#******************************************************************
#
# PROGRAM SETTINGS
...
require_once ($g_lang_dir . "/PS00S01000650M.php");			# message
	...
	for($i=1; $i<=$gw_scr['s_hdn_inp_cnt']; $i++){
		...
		if($w_lot_type_cnt != 0){
			if($w_lot_inp_cnt != $w_lot_type_cnt){
				list($g_msg,$g_err_lv) = PS00S01000650_msg("err_lot_type_cd");
				$g_msg = xpt_err_msg($g_msg, "", __LINE__);
				return 4000;
			}
		}
			        ...
			        return 4000;
	                 ...
	                 #Get the SNI PO and SNI PO Line NO
                         ...
                         if(count($w_sni_po_no) > 1){
                         	list($g_msg, $g_err_lv) = PS00S01000650_msg("err_Multiple_SNI_PO");
                                $g_msg = xpt_err_msg($g_msg, $gw_scr['s_list_lot_id'][$i], __LINE__);
                                return 4000;
                         }
                         ...
                         if($w_sni_po_cnt == 0){
                         	...
                         	if($w_base_sni_po != $w_sni_po_no[0]){
                                	list($g_msg, $g_err_lv) = PS00S01000650_msg("err_SNI_PO_notMatch");
...
function itm($var)
{
	return PS00S01000650_item($var);
}
function msg($var)
{
	return PS00S01000650_msg($var);
}
#******************************************************************
#******************************************************************
#******************************************************************
