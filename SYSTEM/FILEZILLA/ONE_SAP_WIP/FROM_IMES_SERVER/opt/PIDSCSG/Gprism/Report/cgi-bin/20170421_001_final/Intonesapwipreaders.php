...
$this->w_file_name_no_na='/opt/htdocs/interfaces/send/wip_onesap/usg2_onesap_wip_noNA.csv'; # File for content without #N/A
...
function save_file($w_file, $w_msg, $w_date = true){
        ...
        if(is_array($w_msg)){
                ...
                $w_tmp_data_row .= $w_msg['prd_nm'].",";                                # PART_NAME
...
function put_csv($w_header, $service_type, $w_file_name) {
        ...
        $remove_na = $this->remove_na($unfilter_flowtype_e);
        ...
        // }
        for ($i=0; $i<count($unfilter_flowtype_e); $i++) {
                $this->save_file($this->w_file_name, $unfilter_flowtype_e[$i], false);
        }
        $w_date = date('Ymd_His');
