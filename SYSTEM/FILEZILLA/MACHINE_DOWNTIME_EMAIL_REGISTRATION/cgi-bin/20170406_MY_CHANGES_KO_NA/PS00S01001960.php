...
# [SYS_ID]: GPRISM              [SYSTEM]  : Èó¼«Æ°²½É¸½à£Ã£É£Í
# [SUB_ID]:                     [SUBSYS]  :
# [PRC_ID]:                     [PROCESS] :
# [PGM_ID]: PS00S01001960.php   [PROGRAM] : ¥×¥í¥°¥é¥à¥Þ¥¹¥¿ÁªÂò¡¿ÅÐÏ¿
# [MDL_ID]:                     [MODULE]  :
# -------------------------------------------------------------------------------
# [COMMENT]
#
...
# ====================  ==================  =============================================
# ORDER BYÂÐ±þ			2005/03/03			Oracle -> InformixÂÐ±þ 		DOS)Y.Kawakami
# DOS)K.Yamamoto        ID-2007-04-04-002   Ç§¾Úµ¡Ç½¥Õ¥é¥°$refe_flg»ØÄê¸í¤ê½¤Àµ
# -------------------------------------------------------------------------------
$g_Version = "2.0";
...
require_once (getenv("GPRISM_HOME") . "/Func/Check.php");					# ÆþÎÏ·¿¥Á¥§¥Ã¥¯´Ø¿ô·²

require_once ($g_Mfunc_dir . "/global.php");									# ¶¦ÄÌÊÑ¿ô
require_once ($g_Mfunc_dir . "/xglob_pm.php");								# ¶¦ÄÌÊÑ¿ô(¹©Äø´ÉÍý)
require_once ($g_Mfunc_dir . "/db_op.php");									# £Ä£ÂÁàºî
require_once ($g_Mfunc_dir . "/xdb_op.php");									# £Ä£ÂÀÜÂ³´ØÏ¢
require_once ($g_Mfunc_dir . "/xpt_err_msg.php");							# ¥¨¥é¡¼¥á¥Ã¥»¡¼¥¸ºîÀ®´Ø¿ô
require_once ($g_Mlang_dir . "/buttonM.php");								# ¥Ü¥¿¥óÌ¾¾Î
...
require_once ($g_Gfunc_dir . "/xpt_screen.php");							# ²èÌÌÅ¸³«´Ø¿ô
require_once ($g_lang_dir . "/PS00S01001960M.php");								# ¸À¸ì¥Õ¥¡¥¤¥ë
...
#exit();
	...
	# Áí·ï¿ô¤Î¼èÆÀ
	#-------------------------------------------------
	$w_where_cnt = ereg_replace("ORDER","--ORDER",$w_where);		# ORDER BY ÂÐ±þ 2005/03/03
	...
	$cnt = 0;
	$i = 0;
	$w_end_pt = $gw_scr['s_sel_page'] * (PAGE_MAXROW * PAGE_MAXCOL);     # ¥Ú¡¼¥¸ºÇ½ª¥Ç¡¼¥¿¥Ý¥¤¥ó¥¿
	$w_start_pt = $w_end_pt - (PAGE_MAXROW *  PAGE_MAXCOL) + 1;           # ¥Ú¡¼¥¸³«»Ï¥Ç¡¼¥¿¥Ý¥¤¥ó¥¿
...
function get_page_info(){
	...
	if($w_chg_flg == 0){
		...
		# ¸¡º÷¾ò·ï¤¬ÊÑ¹¹¤µ¤ì¤Æ¤¤¤¿¾ì¹ç¤Ï¡¢£±¥Ú¡¼¥¸ÌÜ¤Ë¥»¥Ã¥È
		$gw_scr['s_sel_page'] = 1;
	}
...
# ¥»¥Ã¥·¥ç¥óÆâ¤Î¥â¡¼¥É¤ò¼èÆÀ¡Ê½é´üÁ«°Ü»þ¤Ï¶õ¤Ë¤¹¤ë¡Ë
get_session_mode();

# Îã³° **************************************************************
# ¥×¥í¥°¥é¥à¥Þ¥¹¥¿ÅÐÏ¿²èÌÌ(PSSEM00101110)¤«¤é°ìÍ÷ÁªÂò¤È¤·¤ÆÁ«°Ü¤·¤Æ¤­¤¿¾ì¹ç
# ÇÛÎó¤ËÅÐÏ¿¤·¤¿SCR_ID¤Ï¡¢Ç§¾Ú¤ò¹Ô¤ï¤Ê¤¤
...
for($ii=0;$ii<count($arr_http_chk);$ii++){
    if(ereg($arr_http_chk[$ii],$http_ref)){
		$refe_flg=1;		# Ç§¾Ú³ÎÇ§¤ò¹Ô¤ï¤º¤ËÅö²èÌÌÇ§¾Ú¼èÆÀ
		break;
	}
}
# *******************************************************************
